...
| `GITHUB_TOKEN` | 複数セット保存・切替を使う場合のPAT（Contents: Read and write） | `github_pat_...` |
| `GITHUB_OWNER` | リポジトリ所有者 | `yourname` |
| `GITHUB_REPO` | リポジトリ名 | `ai-literacy-test` |
| `GITHUB_BRANCH` | 対象ブランチ（省略時 main） | `main` |

> **GitHub連携（複数セットの保存・切替）について**: 管理画面から複数の出題セットをGitHubに保存し、検定で使うものを切り替える機能です。使うには上記 `GITHUB_TOKEN` / `GITHUB_OWNER` / `GITHUB_REPO` を設定します。トークンは **Fine-grained personal access token** を推奨し、権限は対象リポジトリの **Contents: Read and write** のみに絞ってください。トークンはVercelの環境変数に置くためブラウザには露出しません。未設定でも従来のダウンロード方式は使えます。
